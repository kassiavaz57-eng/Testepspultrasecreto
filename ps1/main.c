...
static int probe(void){CdlFILE f;uint8_t sector[2048];if(!CdSearchFile(&f,"\\DATA.WIN;1"))return -1;if(CdRead(1,sector,CdlModeSize1)<0)return -2;if(CdReadSync(0,0)<0)return -3;if(memcmp(sector,"GEN8",4)!=0)return -4;return 0;}
