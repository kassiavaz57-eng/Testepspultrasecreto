...
static void ps1DrawRectangle(Renderer* renderer, float x1, float y1, float x2, float y2, uint32_t color, float alpha, bool outline) {
    ...
    if (outline) {
        int w = bx-ax, h = by-ay;
        t=(TILE*)allocPrim(p,sizeof(TILE)); if(t){setTile(t);setXY0(t,ax,ay);setWH(t,w,1);setRGB0(t,cr(color),cg(color),cb(color));queuePrim(p,t);}
        t=(TILE*)allocPrim(p,sizeof(TILE)); if(t){setTile(t);setXY0(t,ax,by-1);setWH(t,w,1);setRGB0(t,cr(color),cg(color),cb(color));queuePrim(p,t);}
        t=(TILE*)allocPrim(p,sizeof(TILE)); if(t){setTile(t);setXY0(t,ax,ay);setWH(t,1,h);setRGB0(t,cr(color),cg(color),cb(color));queuePrim(p,t);}
        t=(TILE*)allocPrim(p,sizeof(TILE)); if(t){setTile(t);setXY0(t,bx-1,ay);setWH(t,1,h);setRGB0(t,cr(color),cg(color),cb(color));queuePrim(p,t);}
    } else {
        t=(TILE*)allocPrim(p,sizeof(TILE)); if(!t)return;
        setTile(t); setXY0(t,ax,ay); setWH(t,bx-ax,by-ay); setRGB0(t,cr(color),cg(color),cb(color));
        if (alpha < 1.0f) setSemiTrans(t,1);
        queuePrim(p,t);
    }
}
...
static void ps1DrawLine(Renderer* renderer,float x1,float y1,float x2,float y2,float width,uint32_t color,float alpha){
    Ps1Renderer* p=(Ps1Renderer*)renderer; LINE_F2* l=(LINE_F2*)allocPrim(p,sizeof(LINE_F2)); if(!l)return;
    setLineF2(l); setXY2(l,sx(p,x1),sy(p,y1),sx(p,x2),sy(p,y2)); setRGB0(l,cr(color),cg(color),cb(color)); if(alpha<1.0f)setSemiTrans(l,1); queuePrim(p,l); (void)width;
}
...
/* Texture drawing is deliberately kept behind this backend boundary. The next
 * renderer layer will populate PS1 TPage/CLUT entries from ATLAS.BIN and
 * TEXTURES.BIN; these fallback quads keep the vtable total while that cache is
 * being assembled. */
static void ps1DrawSprite(Renderer*r,int32_t t,float x,float y,float ox,float oy,float xs,float ys,float ang,uint32_t c,float a){(void)t;(void)ang;float w=32*xs,h=32*ys;ps1DrawRectangle(r,x-ox*xs,y-oy*ys,x-ox*xs+w,y-oy*ys+h,c,a,false);}
static void ps1DrawSpritePart(Renderer*r,int32_t t,int32_t sx0,int32_t sy0,int32_t sw,int32_t sh,float x,float y,float xs,float ys,float ang,float px,float py,uint32_t c,float a){(void)t;(void)sx0;(void)sy0;(void)ang;ps1DrawRectangle(r,x-px*xs,y-py*ys,x-px*xs+sw*xs,y-py*ys+sh*ys,c,a,false);}
...
static void ps1DrawSpritePos(Renderer*r,int32_t t,float x1,float y1,float x2,float y2,float x3,float y3,float x4,float y4,float a){(void)t;(void)x3;(void)y4;ps1DrawRectangle(r,x1,y1,x4,y4,0xFFFFFF,a,false);}
static void ps1DrawSpriteTiled(Renderer*r,int32_t t,float ox,float oy,float x,float y,float xs,float ys,bool tx,bool ty,float rw,float rh,uint32_t c,float a){(void)t;(void)ox;(void)oy;(void)tx;(void)ty;(void)rw;(void)rh;ps1DrawSprite(r,0,x,y,0,0,xs,ys,0,c,a);}
static void ps1DrawTile(Renderer*r,RoomTile*t,float ox,float oy){(void)r;(void)t;(void)ox;(void)oy;}
static void ps1DrawTiledPart(Renderer*r,int32_t t,int32_t x,int32_t y,int32_t w,int32_t h,float dx,float dy,float dw,float dh,uint32_t c,float a){(void)t;(void)x;(void)y;ps1DrawRectangle(r,dx,dy,dx+dw,dy+dh,c,a,false);}
