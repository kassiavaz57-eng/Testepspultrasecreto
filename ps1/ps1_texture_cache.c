...
static bool readAtlasHeader(Ps1TextureCache* cache, FILE* f) {
    ...
    repeat(atlasCount, i) {
        ...
        if (a->width == 0 || a->height == 0 || a->width > 1024 || a->height > 1024) return false;
    }
...
static void decompressAtlas(const uint8_t* src,uint32_t srcSize,uint8_t compression,uint32_t outSize,uint8_t* out) {
    if(compression==1){
        ...
        while(s+1<srcSize&&d<outSize){
            uint8_t count=src[s++],value=src[s++];
            repeat(count,j){if(d>=outSize)break;out[d++]=value;}
        }
...
static int findFreeTextureSlot(Ps1TextureCache* cache){
    ...
    uint32_t oldest=0xFFFFFFFFu;int victim=0;
...
uint16_t Ps1TextureCache_getTPage(Ps1TextureCache* cache,uint16_t atlasId){
    ...
    repeat(PS1_TEXTURE_MAX_SLOTS,i)if(cache->textures[i].valid&&cache->textures[i].atlasId==atlasId){cache->textures[i].lastUsed=++cache->frameCounter;return getTPage(cache->textures[i].bpp==4?0:1,0,cache->textures[i].x,cache->textures[i].y);}
    if(!loadAtlas(cache,atlasId))return 0;
    repeat(PS1_TEXTURE_MAX_SLOTS,i)if(cache->textures[i].valid&&cache->textures[i].atlasId==atlasId)return getTPage(cache->textures[i].bpp==4?0:1,0,cache->textures[i].x,cache->textures[i].y);
    return 0;
}
...
uint16_t Ps1TextureCache_getClut(Ps1TextureCache* cache,uint8_t bpp,uint16_t clutIndex){
    ...
    repeat(count,i)if(slots[i].valid&&slots[i].clutIndex==clutIndex){slots[i].lastUsed=++cache->frameCounter;return getClut(slots[i].x,slots[i].y);}
    return 0;
}
