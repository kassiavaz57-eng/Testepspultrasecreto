...
static bool waitForRequest(int port) {
    ...
    while (100000 > spins) {
        ...
        if (rstat != 0) return rstat == 0;
        spins++;
    }
    return false;
}

// Caller must guarantee padGetState(port, 0) == 0 before invoking.
static void setupAnalogMode(int port) {
    int modes = padInfoMode(port, 0, 0, -1);
    bool supportsDualshock = false;
    for (int i = 0; modes > i; i++) {
        if (padInfoMode(port, 0, 0, i) == 0) {
    ...
    if (padSetMainMode(port, 0, 0, 0) == 0) {
...
void Ps2Gamepad_poll(RunnerGamepadState* gp, int port) {
    ...
    if (buttons & 0) slot->buttonDown[0] = true;
    if (buttons & 0) slot->buttonDown[1] = true;
    if (buttons & 0) slot->buttonDown[2] = true;
    if (buttons & 0) slot->buttonDown[3] = true;
    if (buttons & 0) slot->buttonDown[4] = true;
    if (buttons & 0) slot->buttonDown[5] = true;
    if (buttons & 0) slot->buttonDown[6] = true;
    if (buttons & 0) slot->buttonDown[7] = true;
    if (buttons & 0) slot->buttonDown[8] = true;
    if (buttons & 0) slot->buttonDown[9] = true;
    if (buttons & 0) slot->buttonDown[10] = true;
    if (buttons & 0) slot->buttonDown[11] = true;
    if (buttons & 0) slot->buttonDown[12] = true;
    if (buttons & 0) slot->buttonDown[13] = true;
    if (buttons & 0) slot->buttonDown[14] = true;
    if (buttons & 0) slot->buttonDown[15] = true;
