...
static uint32_t bgrToGu(uint32_t c, float alpha) {
    unsigned int r = BGR_R(c);
    unsigned int g = BGR_G(c);
    unsigned int b = BGR_B(c);
    unsigned int a = (unsigned int)(alpha * 255.0f);
    if (a > 255) a = 255;
    return GU_RGBA(r, g, b, a);
}

static void setOrtho(float left, float right, float top, float bottom, int portX, int portY, int portW, int portH) {
    sceGuViewport(2048 + portX + portW / 2, 2048 + portY + portH / 2, portW, portH);
    sceGuScissor(portX, portY, portX + portW, portY + portH);
    sceGumMatrixMode(GU_PROJECTION);
    sceGumLoadIdentity();
    sceGumOrtho(left, right, bottom, top, -1.0f, 1.0f);
...
static bool loadPage(DataWin *dw, int pageId) {
    ...
    uint8_t *pixels = ImageDecoder_decodeToRgba(tex->blobData, tex->blobSize, gm2022_5, &w, &h);
    if (!pixels || w <= 0 || h <= 0) {
        free(pixels);
        return false;
    }
    ...
    /* Keep a single decoded page cached only when it is reasonably sized.
       Large pages are still rendered, but are released after the draw. */
    if (size <= (size_t)8 * 1024 * 1024) {
        g_cachedPixels = pixels;
        g_cachedPixelsSize = size;
        g_cachedPage = pageId;
        g_cachedW = w;
        g_cachedH = h;
        return true;
...
static void releaseLargePageCache(void) {
    if (g_cachedPixels && g_cachedPixelsSize > (size_t)8 * 1024 * 1024)
        cacheClear();
}
...
static bool uploadRect(DataWin *dw, int pageId, int sx, int sy, int sw, int sh, int *twOut, int *thOut) {
    ...
    if (!loadPage(dw, pageId)) return false;

    if (sx < 0 || sy < 0 || sx + sw > g_cachedW || sy + sh > g_cachedH) return false;

    int tw = nextPow2(sw);
    int th = nextPow2(sh);
    if (tw > PSP_TEX_MAX || th > PSP_TEX_MAX) return false;

    memset(g_textureScratch, 0, sizeof(g_textureScratch));
    for (int y = 0; y < sh; ++y) {
        memcpy(g_textureScratch + (size_t)y * PSP_TEX_MAX * 4,
               g_cachedPixels + ((size_t)(sy + y) * (size_t)g_cachedW + (size_t)sx) * 4,
               (size_t)sw * 4);
    }

    sceKernelDcacheWritebackAll();
    sceGuTexMode(GU_PSM_8888, 0, 0, GU_FALSE);
    sceGuTexImage(0, tw, th, PSP_TEX_MAX, g_textureScratch);
...
static void drawQuad(float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3,
    ...
    v[0] = (PSPVertex){u0, v0, c0, x0, y0, 0};
    v[1] = (PSPVertex){u1, v0, c1, x1, y1, 0};
    v[2] = (PSPVertex){u1, v1, c2, x2, y2, 0};
    v[3] = (PSPVertex){u0, v1, c3, x3, y3, 0};
    sceGuDrawArray(GU_TRIANGLE_FAN, GU_TEXTURE_32BITF | GU_COLOR_8888 | GU_VERTEX_32BITF | GU_TRANSFORM_2D, 4, NULL, v);
}

static void pspDrawSpritePartColor(Renderer *renderer, int32_t tpagIndex, int32_t srcOffX, int32_t srcOffY, int32_t srcW, int32_t srcH,
                                   float x, float y, float xscale, float yscale, float angleDeg, float pivotX, float pivotY,
                                   uint32_t color1, uint32_t color2, uint32_t color3, uint32_t color4, float alpha) {
    DataWin *dw = renderer->dataWin;
    if (!dw || tpagIndex < 0 || (uint32_t)tpagIndex >= dw->tpag.count) return;
    TexturePageItem *tpag = &dw->tpag.items[tpagIndex];
    int pageId = tpag->texturePageId;
    int sx = (int)tpag->sourceX + srcOffX;
    int sy = (int)tpag->sourceY + srcOffY;

    int tw, th;
    if (!uploadRect(dw, pageId, sx, sy, srcW, srcH, &tw, &th)) return;

    float u0 = 0.0f, v0 = 0.0f;
    float u1 = (float)srcW, v1 = (float)srcH;

    float qx[4] = {x, x + srcW*xscale, x + srcW*xscale, x};
    float qy[4] = {y, y, y + srcH*yscale, y + srcH*yscale};
    ...
    if (angleDeg != 0.0f) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            qx[i] = ca * dx - sa * dy + pivotX;
            qy[i] = sa * dx + ca * dy + pivotY;
        }
    }

    drawQuad(qx[0], qy[0], qx[1], qy[1], qx[2], qy[2], qx[3], qy[3],
             u0, v0, u1, v1,
             bgrToGu(color1, alpha), bgrToGu(color2, alpha),
             bgrToGu(color3, alpha), bgrToGu(color4, alpha));
}

static void pspDrawSpritePart(Renderer *renderer, int32_t tpagIndex, int32_t srcOffX, int32_t srcOffY, int32_t srcW, int32_t srcH,
                              float x, float y, float xscale, float yscale, float angleDeg, float pivotX, float pivotY,
                              uint32_t color, float alpha) {
    pspDrawSpritePartColor(renderer, tpagIndex, srcOffX, srcOffY, srcW, srcH, x, y, xscale, yscale, angleDeg, pivotX, pivotY,
                           color, color, color, color, alpha);
}

static void pspDrawSprite(Renderer *renderer, int32_t tpagIndex, float x, float y, float originX, float originY,
    ...
    float px = x - originX * xscale;
    float py = y - originY * yscale;
    pspDrawSpritePart(renderer, tpagIndex, 0, 0, tpag->sourceWidth, tpag->sourceHeight,
                      px, py, xscale, yscale, angleDeg, x, y, color, alpha);
}
...
static void pspDrawRectangle(Renderer *renderer, float x1, float y1, float x2, float y2, uint32_t color, float alpha, bool outline) {
    (void)renderer;
    uint32_t c = bgrToGu(color, alpha);
    PSPVertex *v = (PSPVertex *)sceGuGetMemory(4 * sizeof(PSPVertex));
    v[0] = (PSPVertex){0,0,c,x1,y1,0};
    v[1] = (PSPVertex){0,0,c,x2,y1,0};
    v[2] = (PSPVertex){0,0,c,x2,y2,0};
    v[3] = (PSPVertex){0,0,c,x1,y2,0};
    sceGuDisable(GU_TEXTURE_2D);
    sceGuDrawArray(outline ? GU_LINE_STRIP : GU_TRIANGLE_FAN, GU_COLOR_8888 | GU_VERTEX_32BITF | GU_TRANSFORM_2D, 4, NULL, v);
    sceGuEnable(GU_TEXTURE_2D);
}

static void pspClearScreen(Renderer *renderer, uint32_t color, float alpha) {
    (void)renderer;
    sceGuClearColor(bgrToGu(color, alpha));
...
Renderer *PSPRenderer_create(void) {
    ...
    g_baseVtable = renderer->vtable;
    g_pspVtable = (RendererVtable *)malloc(sizeof(RendererVtable));
    if (!g_pspVtable) return renderer;
    memcpy(g_pspVtable, g_baseVtable, sizeof(RendererVtable));

    g_pspVtable->init = pspInit;
    g_pspVtable->destroy = pspDestroy;
    g_pspVtable->beginFrame = pspBeginFrame;
    g_pspVtable->endFrameInit = pspEndFrameInit;
    g_pspVtable->endFrameEnd = pspEndFrameEnd;
    g_pspVtable->beginView = pspBeginView;
    g_pspVtable->endView = pspEndView;
    g_pspVtable->beginGUI = pspBeginGUI;
    g_pspVtable->setGuiProjection = pspSetGuiProjection;
    g_pspVtable->endGUI = pspEndGUI;
    g_pspVtable->drawSprite = pspDrawSprite;
    g_pspVtable->drawSpritePart = pspDrawSpritePart;
    g_pspVtable->drawSpritePartColor = pspDrawSpritePartColor;
    g_pspVtable->drawRectangle = pspDrawRectangle;
    g_pspVtable->clearScreen = pspClearScreen;

    renderer->vtable = g_pspVtable;
    return renderer;
}
