...
for(;;){
    ...
    /* Diagnostic: overwrite the completed frame with a solid magenta clear.
       This uses only the GU clear/swap path, so it cannot depend on textures,
       vertices, matrices, or the Runner renderer. */
    sceGuClearColor(GU_RGBA(255,0,255,255));
