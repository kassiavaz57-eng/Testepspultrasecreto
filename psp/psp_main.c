...
Runner_drawGUI(runner, 480, 272, (int32_t)d->gen8.defaultWindowWidth, (int32_t)d->gen8.defaultWindowHeight);
sceGuFinish();
sceGuSync(GU_SYNC_FINISH, GU_SYNC_WHAT_DONE);
sceDisplayWaitVblankStart();
sceGuSwapBuffers();
next+=step;uint64_t now=nowNanos();if(next>now)sceKernelDelayThread((unsigned int)((next-now)/1000ULL));else next=now;}sceKernelExitGame();return 0;}